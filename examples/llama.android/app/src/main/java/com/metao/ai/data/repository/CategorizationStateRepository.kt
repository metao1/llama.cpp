...
interface CategorizationStateRepository {
    ...
    suspend fun saveCategorizationResults(
        sessionId: String,
        results: List<CategorizationResult>,
    )

    suspend fun saveMoveOperations(
        sessionId: String,
    ...
    suspend fun markMoveOperationExecuted(
        sessionId: String,
        operationId: String,
        success: Boolean,
        result: String,
...
class CategorizationStateRepositoryImpl(
    ...
    override suspend fun getLastSession(): CategorizationSessionEntity? = dao.getLastSession()
    ...
    override suspend fun saveCategorizationResults(
        sessionId: String,
        results: List<CategorizationResult>,
    ) {
    ...
    override suspend fun saveMoveOperations(
        sessionId: String,
        operations: List<MoveOperation>,
    ) {
        val entities =
    ...
    override suspend fun getCategorizationResults(sessionId: String): Flow<List<CategorizationResult>> =
        dao.getCategorizationResultsForSession(sessionId).map { entities ->
            entities.map { it.toDomainModel() }
        }

    override suspend fun getMoveOperations(sessionId: String): Flow<List<MoveOperation>> =
        dao.getMoveOperationsForSession(sessionId).map { entities ->
            entities.map { it.toDomainModel() }
        }

    override suspend fun updateMoveOperationSelection(
        sessionId: String,
        operationIndex: Int,
        isSelected: Boolean,
    ) {
    ...
    override suspend fun updateAllMoveOperationsSelection(
        sessionId: String,
        isSelected: Boolean,
    ) {
    ...
    override suspend fun markMoveOperationExecuted(
        sessionId: String,
        operationId: String,
        success: Boolean,
        result: String,
